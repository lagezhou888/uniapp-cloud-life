		 ...
		 :style="{
			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap" @tap.stop.prevent="listClick(index)" :class="[current != index ? 'u-list-scale' : '']" :style="{
					...
					<image v-if="!isLocation" class="u-swiper-image" :src="item[name]" :mode="imgMode"></image>
					<image v-if="isLocation" class="u-swiper-image" :src="currImgSrc" :mode="imgMode"></image>
...
<script>
	...
	export default {
		...
		watch: {
			...
			currentPostion(nVal, oVal){
				this.isLocation = true;
				this.current = nVal.index;
				this.currImgSrc = nVal.src;
			}
		},
		data() {
			return {
